...
import { useCopyToClipboard } from "~/components/function/data-table/components/src/hooks/use-copy-to-clipboard";
import { composeRefs } from "~/components/function/data-table/components/src/lib/compose-refs";
...
>(({ children, variant, maxHeight, className, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      {collapsible && !open ? (
        <div className="absolute inset-x-px bottom-px flex items-center justify-center rounded-b-md bg-linear-to-b from-background/0 to-background">
          <Button
            variant="outline"
            size="sm"
