...
import { InputWithAddons } from "~/components/function/data-table/components/src/custom/input-with-addons";
import { Label } from "~/components/function/text/label";
import { Slider } from "~/components/function/data-table/components/src/custom/slider";
import { isArrayOfNumbers } from "~/components/function/data-table/components/src/lib/is-array";
import { useEffect, useState } from "react";
import { useDebounce } from "~/components/function/data-table/components/src/hooks/use-debounce";
...
export function DataTableFilterSlider<TData>({
  ...
  useEffect(() => {
    if (!filters) {
      setInput(null);
    } else if (filters && (!input || debouncedInput?.[0] !== filters[0] || debouncedInput?.[1] !== filters[1])) {
      setInput(filters);
    }
  }, [filters, input, debouncedInput]);
