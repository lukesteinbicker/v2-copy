...
import {
  ...
  ChartTooltip,
  ChartTooltipContent,
} from "~/components/function/misc/chart";
import { useMemo, useState } from "react";
import { format } from "date-fns";
import { cn } from "~/components/function/data-table/components/src/lib/utils";
import type { CategoricalChartFunc } from "recharts/types/chart/generateCategoricalChart";
import { getLevelLabel } from "~/components/function/data-table/components/src/lib/request/level";
...
export function TimelineChart<TChart extends BaseChartSchema>({
  ...
  const chart = useMemo(
    () =>
      data.map((item) => ({
        ...item,
        [columnId]: new Date((item as unknown as { timestamp: number }).timestamp).toString(),
      })),
    [data, columnId],
  );
  ...
  const timerange = useMemo(() => {
    ...
    const first = (data[0] as unknown as { timestamp: number }).timestamp;
    const last = (data[data.length - 1] as unknown as { timestamp: number }).timestamp;
  ...
  const handleMouseUp: CategoricalChartFunc = (_e) => {
...
function TooltipLabel({
  ...
  level: keyof Omit<BaseChartSchema, "timestamp">;
}) {
  return (
