...
import { getLevelRowClassName } from "~/components/function/data-table/components/src/lib/request/level";
...
import { DataTableFilterField, SheetField } from "~/components/function/data-table/components/src/data-table/types";
import { ColumnSchema } from "./types";

interface ClientPageProps {
  columns: ColumnDef<any>[];
  defaultFilterFields: DataTableFilterField<any>[];
  sheetFields: SheetField<any, any>[];
  // Schema types that users define for their specific use case
  columnSchema: any;
  baseChartSchema: any;
  facetMetadataSchema: any;
}
...
export function Client({ 
  ...
  const {
    ...
    fetchPreviousPage,
    refetch,
  } = useInfiniteQuery(dataOptions(search, columnSchema, baseChartSchema, facetMetadataSchema, "/datatable-infinite/api"));
  useResetFocus();
  ...
  const filterFields = React.useMemo(() => {
    return defaultFilterFields.map((field) => {
      ...
      const options = facetsField.rows.map(({ value, total }: { value: any; total: any }) => {
        return {
          label: `${value}`,
          value,
        };
  ...
  return (
    <DataTableInfinite
      ...
      defaultColumnSorting={sort ? [{ id: sort.id || '', desc: sort.desc }] : undefined}
      ...
      renderLiveRow={(props) => {
        ...
        return <LiveRow columns={columns as ColumnDef<ColumnSchema>[]} />;
      }}
      renderSheetTitle={(props) => props.row?.original.pathname}
      searchParamsParser={searchParamsParser as any}
    />
  );
}
...
export function getFacetedUniqueValues<TData>(
  ...
  return (_: TTable<TData>, columnId: string): Map<string, number> => {
    return new Map(
      facets?.[columnId]?.rows?.map(({ value, total }: { value: any; total: any }) => [value, total]) || [],
    );
  };
}
